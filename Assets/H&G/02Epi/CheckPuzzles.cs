 ...
 *
 * <Variable>
 * mgo_GameControl              "AnswerCheck" 스크립트 연결을 위한 변수
...
public class CheckPuzzles : MonoBehaviour{
    private GameObject mgo_GameControl;                                                                             //AnswerCheck 스크립트 연결을 위한 변수
    ...
    private void OnTriggerStay2D(Collider2D col){
        if (Input.GetMouseButtonUp(0)){                                                                             //만약 화면에서 마우스 클릭을 떼면
            if (col.tag == this.tag){                                                                               //충돌체의 태그와 현재 오브젝트(퍼즐조각)의 태그가 같다면
                ...
                mgo_GameControl.GetComponent<AnswerCheck>().v_CountAnswer();                                        //정답 개수를 올려주기 위한 함수 실행
            }
        }
    }
}
